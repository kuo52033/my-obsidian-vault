...
a number of unique values in a row
...
![[Pasted image 20260314224622.png]]
...
![[Pasted image 20260314224633.png]]
...
1. **Query-driven** — design indexes based on actual queries (WHERE, JOIN, ORDER BY, SELECT), not the table structure
2. **High selectivity columns on the left** of composite indexes
3. **Equality conditions before range conditions** in composite index column order
4. **Covering index must be complete** — include all columns from WHERE, JOIN, SELECT, ORDER BY; missing any one causes Table Lookup
5. **Be aware of low-selectivity columns** — MySQL may ignore the index and do a full table scan if selectivity is too low
6. **Indexes have write overhead** — every INSERT/UPDATE/DELETE must also update all relevant indexes; don't over-index