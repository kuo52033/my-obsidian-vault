...
Stream: chat:room1
┌─────────────────────────────────────────┐
│ 1700000001-0  { from: 'A', text: 'hi' } │
│ 1700000002-0  { from: 'B', text: 'hey' }│
│ 1700000003-0  { from: 'A', text: 'yo' } │
└─────────────────────────────────────────┘
```

- 訊息**永久存在 Stream 裡**（除非你手動刪）
- 每筆訊息有唯一 ID（timestamp + sequence）
- Consumer 可以指定從哪個 ID 開始讀
- 斷線重連後，從上次讀到的位置繼續
- 有確認機制  ACK
